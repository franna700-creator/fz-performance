...
function v2Readiness(r){const score=Number.isFinite(Number(r?.score))?Number(r.score):null;return `<aside class="fz2-readiness"><div class="fz2-readiness-score"><strong>${score??'—'}</strong><span>READINESS</span></div><div class="fz2-readiness-copy"><small>${v2Esc(r?.status||'CURRENT STATE')}</small><p>${v2Esc(r?.systemicRecovery||'Current recovery interpretation is unavailable.')}</p></div></aside>`}
...
function v2Training(focus){if(!focus)return `<section class="fz2-training"><div><span>TRAINING</span><h3>No current canonical training session.</h3></div></section>`;const event=(focus.events||[]).filter(e=>e.actor==='ATHLETE').sort((a,b)=>new Date(b.occurred_at)-new Date(a.occurred_at))[0];return `<section class="fz2-training"><div><span>LAST KEY EXECUTION</span><h3>${v2Esc(focus.title||focus.sport_type||'Training')}</h3>${event?`<p>${v2Esc(event.summary)}</p>`:''}</div><button class="fz2-open-train" data-open-page="train">OPEN TRAIN →</button></section>`}
...
function v2Mount(){
  ...
  shell.innerHTML=v2ShellHtml(r,well,w,focus);
...
const v2Observer=new MutationObserver(()=>{const root=document.getElementById('today');if(root&&!root.querySelector(':scope > .fz2-shell')&&!FZ_TODAY_V2.busy)scheduleV2Load(30)});
