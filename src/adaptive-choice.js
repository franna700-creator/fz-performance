const FZ_CHOICE={current:null,loading:false,renderScheduled:false,rendering:false};
...
function choiceFingerprint(){
  ...
  return [current.revision||'no-revision',rec?.recommendationVersion||rec?.shadowRecommendationId||rec?.fzRecommendedLane||'no-recommendation',selected?.decisionId||selected?.optionId||selected?.selectedLane||'no-decision'].join('|');
}
...
function renderTrainChoice(){
  ...
  section.innerHTML=`<div class="section-head"><h2>Current Training Choice</h2><p>FZ recommendation → concrete options → athlete decision → planned intent → execution reconciliation</p></div><div class="fz-choice-summary"><div class="card decision rich"><div class="eyebrow">FZ RECOMMENDED LANE${rec.localDate?` · ${esc(rec.localDate)}`:''}</div><h3>${esc(rec.status==='READY'?rec.fzRecommendedLane:'WITHHELD')}</h3><p>${esc(whyNow())}</p><div class="microline"><strong>Objective connection:</strong> ${esc(objectiveConnection())}</div></div><div class="card rich"><div class="eyebrow">ATHLETE DECISION</div>${selected?`<h3>${esc(selected.option?.title||selected.optionId||'Recorded choice')}</h3><p>${esc(words(selected.selectedLane))}${selected.matchesFzRecommendation?' · accepted FZ direction':' · athlete override retained'}</p><div class="microline"><strong>Planned:</strong> ${esc(selected.plannedForDate||'—')} · <strong>Plan:</strong> ${esc(selected.plannedSessionId||'—')}</div>`:'<h3>Not selected yet</h3><p>Your choice remains separate from FZ’s recommendation until you act.</p><div class="microline">Choose naturally in chat; FZ will preserve the recommendation and your decision separately.</div>'}</div></div>${options.length?`<div class="fz-options-grid">${options.map(optionCard).join('')}</div>`:'<div class="card rich"><h3>No concrete session option is currently released.</h3><p>FZ has withheld session composition because the current recommendation context does not support it.</p></div>'}<details class="fz-alternate-lanes"><summary>See alternate lanes</summary><div class="fz-alternate-lane-list">${alternateLanes.map(lane=>alternateLaneBlock(rec,lane)).join('')}</div></details>`;
}
...
function render(){
  ...
  observer.disconnect();
  try{removeDuplicateSections();patchToday();renderTrainChoice();rationaliseSystem();}
