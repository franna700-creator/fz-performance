...
function gpRender(){
  ...
  root.innerHTML=`<div class="fz-goals-stage">
    ...
    <section class="fz-goals-section"><header><div><span>CAPABILITY PROGRESS</span><h2>What the evidence says</h2></div><p>Current qualified capability evidence and next observations. Evidence availability does not establish improvement.</p></header>${capabilities.length?`<div class="fz-goals-capabilities">${capabilities.map(gpCapabilityCard).join('')}</div>`:'<div class="fz-goals-empty-line">Capability trajectory is not currently available.</div>'}</section>
