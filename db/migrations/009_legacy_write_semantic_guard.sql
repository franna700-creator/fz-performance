...
-- Tranche 5.0 release hardening for legacy clients that may continue to call an
-- older immutable deployment after the canonical alias has moved.
--
-- 1) Reconciliation confidence is monotonic at the persistence boundary.
-- 2) The mutation outbox records semantic changes, not idempotent sync touches.

CREATE OR REPLACE FUNCTION fz_preserve_training_reconciliation_monotonic()
RETURNS TRIGGER AS $$
BEGIN
  IF OLD.reconciliation_state = 'MANUAL' THEN
    NEW.reconciliation_state := 'MANUAL';
  ELSIF OLD.reconciliation_state = 'MATCHED'
        AND NEW.reconciliation_state IN ('UNMATCHED','MATCH_REQUIRED') THEN
    NEW.reconciliation_state := 'MATCHED';
  ELSIF OLD.reconciliation_state = 'MATCH_REQUIRED'
        AND NEW.reconciliation_state = 'UNMATCHED' THEN
    NEW.reconciliation_state := 'MATCH_REQUIRED';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS fz_preserve_training_reconciliation ON fz_training_sessions;
CREATE TRIGGER fz_preserve_training_reconciliation
BEFORE UPDATE OF reconciliation_state ON fz_training_sessions
FOR EACH ROW EXECUTE FUNCTION fz_preserve_training_reconciliation_monotonic();
...
BEGIN
  ...
  -- Ignore SQL UPDATEs that do not change canonical meaning. Legacy training
  -- sync always bumps fz_training_sessions.updated_at, so that housekeeping
  -- timestamp is intentionally excluded from semantic equality.
