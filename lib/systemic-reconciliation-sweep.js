...
export async function runLiveSystemicReconciliationSweep({now=new Date()}={}){
  ...
  const [constraintRows,reconciliationRows,stalePlanRows,duplicatePlanRows,unassessedRows,objectiveRows,choiceOutcomeRows,athleteStateRow,convergence,outbox]=await Promise.all([
    sql`
    ...
    readCanonicalMutationOutboxStatus().catch(error=>({unavailable:true,pendingCount:null,error:error instanceof Error?error.message:String(error)}))
  ]);
  ...
  if(outbox?.unavailable)add(findings,'CANONICAL_MUTATION_OUTBOX_UNAVAILABLE','ERROR','Persistence-boundary canonical mutation outbox is unavailable.',{error:outbox.error||null});
  else if(Number(outbox?.pendingCount||0)>0)add(findings,'CANONICAL_MUTATIONS_PENDING','ERROR',`${outbox.pendingCount} canonical mutation(s) have not yet been assigned to a revision.`,outbox);
  ...
  return {
    ...
    invariants:{
      choiceOutcomeSchemaSupported:constraintText.includes('choice_outcome'),
      choiceOutcomeRecords:Number(choiceOutcomeRows?.[0]?.count||0),
      mutationOutbox:outbox,
      decisionObjectives:objectiveRows||[],
      currentAthleteStateFingerprint:athleteStateRow?.payload?.inputFingerprint||null,
