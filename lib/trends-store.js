    ...
    sql`
      ...
      WHERE s.local_date BETWEEN ${startDate} AND ${endDate}
        ...
        AND (s.actual_start_at IS NOT NULL OR EXISTS (
          ...
          WHERE lx.session_id=s.session_id AND lx.relationship='EXECUTION'
        ))
...
export async function buildCurrentTrends({ days = 45 } = {}) {
  ...
  const historicalFallback = date => historicalLoad.has(date)
    ...
    : legacyLoad.has(date)
      ? { value: legacyLoad.get(date), state: legacyLoad.get(date) === 0 ? 'LEGACY_VERIFIED_ZERO' : 'LEGACY_RECONCILED', provenance: 'retained historical runtime fallback' }
      : null;
  ...
  const loadSeries = [...byDate.entries()].map(([date, day]) => {
    ...
    if (!day.length) {
      ...
      return { date, dateLabel: labelDate(date), value: 0, state: 'VERIFIED_ZERO', sessions: 0, provenance: 'canonical execution absence' };
    }
  ...
  return {
    ...
    provenance: {
      ...
      runtimeUsage: 'historical load fallback only; current narratives and capabilities are not sourced from runtime snapshots'
    },
