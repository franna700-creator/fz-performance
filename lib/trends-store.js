  ...
  const loadSeries = [...byDate.entries()].map(([date, day]) => {
    if (!day.length) {
      ...
      if (legacy !== undefined && legacy !== null) {
        return { date, dateLabel: labelDate(date), value: legacy, state: legacy === 0 ? 'LEGACY_VERIFIED_ZERO' : 'LEGACY_RECONCILED', sessions: 0, provenance: 'retained reconciled historical load' };
      }
