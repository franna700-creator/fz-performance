...
export async function refreshIntelligence({ refreshSources = false, forceWellness = false, now = new Date() } = {}) {
  ...
  const steps = {
    ...
    athleteContextIntegrity: null,athleteMaterialityRepair: null,temporalPropagation: null,
  ...
  return {
    ...
    affectedSurfaces: steps.temporalPropagation?.affectedSurfaces||steps.readinessPropagation?.affectedSurfaces||steps.propagation?.affectedSurfaces||current.affectedSurfaces,
