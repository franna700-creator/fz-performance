...
import { readPendingCanonicalMutations, canonicalMutationChangedNodes, acknowledgeCanonicalMutations } from './canonical-mutation-outbox.js';
...
export async function refreshIntelligence({ refreshSources = false, forceWellness = false, now = new Date() } = {}) {
  ...
  const steps = {
    ...
    athleteContextIntegrity: null,athleteMaterialityRepair: null,mutationOutboxPropagation:null,
    bootstrapPropagation: null,temporalPropagation: null,readinessPropagation: null,propagation: null
  };
  ...
  if(current.pending?.mutationOutbox){
    try{
      const mutations=await readPendingCanonicalMutations({limit:500});
      const changedNodes=canonicalMutationChangedNodes(mutations);
      if(changedNodes.length){
        ...
        const mutationIds=mutations.map(row=>Number(row.mutation_id)).filter(Number.isFinite);
        steps.mutationOutboxPropagation=await propagateCanonicalChangeSafely({
          changedNodes,materiality,
          trigger:{type:'CANONICAL_MUTATION_OUTBOX',mutationIds,mutationCount:mutationIds.length,beforeRevision:before.revision},
          now
        });
        if(steps.mutationOutboxPropagation?.canonicalRevisionId){
          await acknowledgeCanonicalMutations({mutationIds,revisionId:steps.mutationOutboxPropagation.canonicalRevisionId});
        }else{
          await acknowledgeCanonicalMutations({mutationIds,error:steps.mutationOutboxPropagation?.error||'mutation_outbox_propagation_failed'});
        }
        warnings.push(...(steps.mutationOutboxPropagation?.warnings||[]));
      }
    }catch(error){warnings.push(`Canonical mutation outbox: ${error instanceof Error?error.message:String(error)}`);}
  ...
  return {
    ...
    convergence:current.convergence,mutationOutbox:current.mutationOutbox,temporal:current.temporal,
    affectedSurfaces: steps.mutationOutboxPropagation?.affectedSurfaces||steps.bootstrapPropagation?.affectedSurfaces||steps.temporalPropagation?.affectedSurfaces||steps.readinessPropagation?.affectedSurfaces||steps.propagation?.affectedSurfaces||current.affectedSurfaces,
