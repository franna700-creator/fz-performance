...
export async function refreshIntelligence({ refreshSources = false, forceWellness = false, now = new Date() } = {}) {
  ...
  if (current.pending.shadowRecommendation) {
    ...
    const shadow = await recomputeRecommendationShadowSafely({
      materiality,
      trigger: {
  ...
  if (current.pending.activeRecommendation) {
    try {
      steps.activeRecommendation = await persistActiveRecommendationFromShadow();
    } catch (error) {
      warnings.push(`Active recommendation projection: ${error instanceof Error ? error.message : String(error)}`);
    }
    current = await readIntelligenceCurrent();
  }
  ...
  return {
    ...
    affectedSurfaces: current.affectedSurfaces,
