...
export async function refreshIntelligence({ refreshSources = false, forceWellness = false, now = new Date() } = {}) {
  ...
  const steps = {
    ...
    athleteContextIntegrity: null,
    athleteMaterialityRepair: null,
    readinessPropagation: null,
    propagation: null
  };
  ...
  try {
    ...
    warnings.push(`Athlete context integrity: ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  if (current.pending.readiness) {
    ...
    steps.readinessPropagation = await propagateCanonicalChangeSafely({
      changedNodes:['wellness.current','recovery.current','athlete.memory','materiality.current'],
      materiality,
      forceRecommendationRecompute:true,
      trigger:{
        type:'INTELLIGENCE_REFRESH_READINESS',
  ...
  return {
    ...
    affectedSurfaces: steps.readinessPropagation?.affectedSurfaces||steps.propagation?.affectedSurfaces||current.affectedSurfaces,
    currentReadiness: current.currentReadiness,
