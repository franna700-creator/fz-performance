...
export const ATHLETE_CURRENT_STATE_ENGINE_VERSION='5.0.0-athlete-state.1';
...
const CONSTRAINT_STALE_AFTER_DAYS=21;
...
const SUBJECT_PATTERNS=Object.freeze([
  ...
  ['CALF',/\bcalf|calves\b/i],
  ['KNEE',/\bknee\b/i],
  ['ANKLE_FOOT',/\bankle|foot|feet\b/i],
  ['QUAD_DOMS',/\bquad|quads|doms|thigh|soreness\b/i],
  ['SHOULDER_UPPER',/\bshoulder|trap|traps|upper back\b/i],
...
function subjects(row){
  const haystack=`${text(row?.summary)} ${text(row?.payload?.rawText)} ${JSON.stringify(row?.payload||{})}`;
  const found=SUBJECT_PATTERNS.filter(([,pattern])=>pattern.test(haystack)).map(([id])=>id);
  return found.length?found:['GENERAL_CONSTRAINT'];
}
function severity(row){
  const reasons=reasonCodes(row);
  if(reasons.includes('SAFETY_RED_FLAG'))return 'SAFETY';
  if(reasons.some(code=>['PAIN_HIGH','DOMS_HIGH','GI_LIMITER_HIGH','CURRENT_CONSTRAINT_HIGH','ATHLETE_RESPONSE_MATERIAL_NEGATIVE'].includes(code)))return 'HIGH';
  if(reasons.some(code=>['PAIN_MODERATE','DOMS_MODERATE','ATHLETE_STATE_MATERIAL_NEGATIVE'].includes(code)))return 'MODERATE';
  return 'REPORTED';
}
function isConstraintEvent(row){
  const cats=categories(row),reasons=reasonCodes(row);
  return cats.includes('CONSTRAINT')||reasons.some(code=>/PAIN|DOMS|GI_LIMITER|CURRENT_CONSTRAINT|SAFETY_RED_FLAG/.test(code));
}
function isResolved(row){return reasonCodes(row).includes('CONSTRAINT_REPORTED_RESOLVED');}
...
function governingObservation(activeConstraints,recentState,recentRecovery){
  ...
  if(active)return {event:active.lastObservation,materiality:active.lastObservation?.materiality||null,reason:'ACTIVE_CONSTRAINT'};
  const recent=recentState||recentRecovery;
  return recent?{event:recent,materiality:recent.materiality||null,reason:'RECENT_STATE'}:{event:null,materiality:null,reason:'NO_CURRENT_ATHLETE_STATE'};
}
...
export function buildCurrentAthleteState({events=[],asOf=new Date()}={}){
  ...
  for(const row of rows){
    if(isResolved(row)){
      let targets=subjects(row).filter(subject=>subject!=='GENERAL_CONSTRAINT');
      if(!targets.length&&active.size===1)targets=[...active.keys()];
    ...
    if(!isConstraintEvent(row))continue;
    for(const subject of subjects(row)){
      const existing=active.get(subject);
  ...
  for(const item of active.values()){
    ...
    if(ageDays>CONSTRAINT_STALE_AFTER_DAYS)staleConstraints.push({...projected,status:'STALE_UNCONFIRMED'});
  ...
  const fingerprintBasis={
    ...
    activeConstraints:activeConstraints.map(item=>({subject:item.subject,severity:item.severity,introducedAt:item.introducedAt,lastConfirmedAt:item.lastConfirmedAt,eventKey:item.lastObservation?.eventKey||null})),
    staleConstraints:staleConstraints.map(item=>({subject:item.subject,severity:item.severity,lastConfirmedAt:item.lastConfirmedAt,eventKey:item.lastObservation?.eventKey||null})),
  ...
  return {
    ...
    readinessContext:{source:governing.reason,athleteEvent:governing.event,materiality:governing.materiality},
    ...
    policy:{constraintStaleAfterDays:CONSTRAINT_STALE_AFTER_DAYS,recentStateHours:RECENT_STATE_HOURS,recentRecoveryHours:RECENT_RECOVERY_HOURS,recentCostHours:RECENT_COST_HOURS,unrelatedFeedbackDoesNotResolveConstraint:true}
  };
}
