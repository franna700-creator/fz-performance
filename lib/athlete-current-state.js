...
const SUBJECT_PATTERNS=Object.freeze([
  ...
  ['QUAD_DOMS',/\b(?:quad|quads|doms|thigh|soreness)\b/i],
  ['SHOULDER_UPPER',/\b(?:shoulder|shoulders|trap|traps|upper back)\b/i],
...
const HEALTHY_CUE=/\b(?:fine|normal|resolved|fully recovered|recovered|pain[- ]?free|symptom[- ]?free|no issues?|no problems?|no symptoms?|no pain|feels? good|felt good)\b/i;
...
function inferredConstraintObservations(row){
  ...
  for(const [subject,pattern] of SUBJECT_PATTERNS){
    ...
    for(const segment of hits){
      ...
      else if(healthy)state='RESOLVED';
