...
function observation(row){
  return {
    eventId:row?.event_id==null?null:Number(row.event_id),eventKey:row?.event_key||null,occurredAt:row?.occurred_at||null,
    localDate:String(row?.local_date||'').slice(0,10)||null,eventType:row?.event_type||null,summary:row?.summary||null,
    categories:categories(row),materiality:materiality(row)
  };
}
function structuredConstraintObservations(row){
  ...
  for(const item of supplied){
    const subject=upper(item?.subject),state=upper(item?.state||item?.status);
...
function explicitlyHealthy(segment,pattern){
  const directNo=new RegExp(`\\bno\\s+(?:current\\s+)?${pattern.source}`,'i');
  const issueWith=new RegExp(`\\bno\\s+(?:issues?|problems?|pain|symptoms?)\\s+(?:with|in)\\s+(?:my\\s+|the\\s+)?${pattern.source}`,'i');
  if(directNo.test(segment)||issueWith.test(segment))return true;
  const notRecovered=new RegExp(`${pattern.source}[^.;]{0,45}\\bnot\\s+(?:fully\\s+)?recovered\\b`,'i');
  if(notRecovered.test(segment))return false;
...
function inferredConstraintObservations(row){
  const reasons=reasonCodes(row),adverse=reasons.some(code=>ADVERSE_REASON_CODES.has(code)),explicitlyResolved=reasons.includes('CONSTRAINT_REPORTED_RESOLVED');
  const observations=[];
  for(const [subject,pattern] of SUBJECT_PATTERNS){
    const hits=subjectSegments(row,pattern);if(!hits.length)continue;
    let state=null;
    for(const segment of hits){
      const healthy=explicitlyHealthy(segment,pattern),active=explicitlyActive(segment,pattern);
...
function constraintObservations(row){const structured=structuredConstraintObservations(row);return structured.length?structured:inferredConstraintObservations(row);}
...
function latestRelevant(rows,category,asOf,hours){const cutoff=rows.filter(row=>categories(row).includes(category)&&hoursBetween(asOf,row.occurred_at)<=hours);return cutoff.length?observation(cutoff[cutoff.length-1]):null;}
...
export function buildCurrentAthleteState({events=[],asOf=new Date()}={}){
  ...
  const active=new Map(),resolutionLog=[];
  for(const row of rows){
    for(const constraint of constraintObservations(row)){
      const subject=constraint.subject;
      if(constraint.state==='RESOLVED'){
        const existing=active.get(subject);if(!existing)continue;
        resolutionLog.push({subject,resolvedBy:observation(row),introducedAt:existing.introducedAt});active.delete(subject);continue;
      }
      const existing=active.get(subject),obs=observation(row);
      active.set(subject,{subject,status:'ACTIVE',severity:constraint.severity,introducedAt:existing?.introducedAt||row.occurred_at,lastConfirmedAt:row.occurred_at,lastObservation:obs,staleAfterDays:staleAfterDays(subject,row),sourceEventKeys:unique([...(existing?.sourceEventKeys||[]),row.event_key])});
    }
  }
  const activeConstraints=[],staleConstraints=[];
  for(const item of active.values()){
    const ageDays=hoursBetween(asOfDate,item.lastConfirmedAt)/24,projected={...item,ageDays:Number(ageDays.toFixed(2))};
    if(ageDays>item.staleAfterDays)staleConstraints.push({...projected,status:'STALE_UNCONFIRMED'});else activeConstraints.push(projected);
  }
  activeConstraints.sort((a,b)=>a.subject.localeCompare(b.subject));staleConstraints.sort((a,b)=>a.subject.localeCompare(b.subject));
  const recentState=latestRelevant(rows,'STATE',asOfDate,RECENT_STATE_HOURS),recentRecovery=latestRelevant(rows,'RECOVERY',asOfDate,RECENT_RECOVERY_HOURS),recentCost=latestRelevant(rows,'COST',asOfDate,RECENT_COST_HOURS);
  const governing=governingObservation(activeConstraints,recentState,recentRecovery);
  const fingerprintBasis={engineVersion:ATHLETE_CURRENT_STATE_ENGINE_VERSION,asOfLocalDate:localDate(asOfDate),activeConstraints:activeConstraints.map(item=>({subject:item.subject,severity:item.severity,introducedAt:item.introducedAt,lastConfirmedAt:item.lastConfirmedAt,staleAfterDays:item.staleAfterDays,eventKey:item.lastObservation?.eventKey||null})),staleConstraints:staleConstraints.map(item=>({subject:item.subject,severity:item.severity,lastConfirmedAt:item.lastConfirmedAt,staleAfterDays:item.staleAfterDays,eventKey:item.lastObservation?.eventKey||null})),recentState:recentState?.eventKey||null,recentRecovery:recentRecovery?.eventKey||null,recentCost:recentCost?.eventKey||null,governingEventKey:governing.event?.eventKey||null,governingConstraint:governing.constraint};
  return {
    schemaVersion:'1.0',contextType:ATHLETE_CURRENT_STATE_CONTEXT,engineVersion:ATHLETE_CURRENT_STATE_ENGINE_VERSION,athleteId:'francois',asOf:asOfDate.toISOString(),localDate:localDate(asOfDate),status:'CURRENT',activeConstraints,staleConstraints,
    recent:{state:recentState,recovery:recentRecovery,cost:recentCost},readinessContext:{source:governing.reason,athleteEvent:governing.event,materiality:governing.materiality,activeConstraint:governing.constraint},resolutionLog:resolutionLog.slice(-20),inputFingerprint:sourceHash(fingerprintBasis),
    provenance:{eventCount:rows.length,latestEventKey:rows.at(-1)?.event_key||null,latestEventAt:rows.at(-1)?.occurred_at||null},
    policy:{defaultConstraintStaleAfterDays:DEFAULT_CONSTRAINT_STALE_AFTER_DAYS,transientDomsStaleAfterDays:TRANSIENT_DOMS_STALE_AFTER_DAYS,giStaleAfterDays:GI_STALE_AFTER_DAYS,illnessStaleAfterDays:ILLNESS_STALE_AFTER_DAYS,recentStateHours:RECENT_STATE_HOURS,recentRecoveryHours:RECENT_RECOVERY_HOURS,recentCostHours:RECENT_COST_HOURS,unrelatedFeedbackDoesNotResolveConstraint:true,constraintMentionDoesNotImplyActiveConstraint:true,negatedHealthyObservationResolvesSubject:true,structuredConstraintObservationsPreferred:true}
  };
}

async function readStateEvents({asOf=new Date(),lookbackDays=60}={}){
  const sql=await getSql();const start=new Date((date(asOf)||new Date()).getTime()-Math.max(1,Number(lookbackDays)||60)*86400000).toISOString();
  return sql`
    SELECT e.event_id,e.event_key,e.event_type,e.occurred_at,e.local_date::text AS local_date,e.certainty,e.summary,e.payload,m.payload AS materiality_payload
    FROM fz_athlete_events e
    LEFT JOIN LATERAL (
      SELECT payload FROM fz_training_source_latest
      WHERE source_key='fz-intelligence' AND record_type='event_context' AND payload->>'contextType'='MATERIALITY_ASSESSMENT' AND payload->>'evidenceKey'=e.event_key
      ORDER BY COALESCE(source_updated_at,ingested_at) DESC,id DESC LIMIT 1
    ) m ON TRUE
    WHERE e.actor='ATHLETE' AND e.occurred_at>=${start} AND e.occurred_at<=${(date(asOf)||new Date()).toISOString()}
...
export async function readCurrentAthleteState(){
  ...
  const rows=await sql`
    ...
    WHERE source_key=${SOURCE_KEY} AND record_type=${RECORD_TYPE} AND source_record_id=${SOURCE_RECORD_ID} AND payload->>'contextType'=${ATHLETE_CURRENT_STATE_CONTEXT}
    ORDER BY COALESCE(source_updated_at,ingested_at) DESC,id DESC LIMIT 1
  `;return rows?.[0]||null;
}

export async function recomputeCurrentAthleteState({now=new Date(),persist=true}={}){
  const before=await readCurrentAthleteState(),events=await readStateEvents({asOf:now}),payload=buildCurrentAthleteState({events,asOf:now});
  ...
  const row=await ingestTrainingSourceRecord({sourceKey:SOURCE_KEY,recordType:RECORD_TYPE,sourceRecordId:SOURCE_RECORD_ID,sourceUpdatedAt:payload.provenance.latestEventAt||now.toISOString(),localDate:payload.localDate,payload});
  return {status:'CURRENT',changed,before:before?.payload||null,row,payload};
}
export async function recomputeCurrentAthleteStateSafely(options={}){try{return await recomputeCurrentAthleteState(options);}catch(error){return {status:'ERROR',changed:false,error:error instanceof Error?error.message:String(error),payload:null,row:null};}}
