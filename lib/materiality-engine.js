...
function normalizeSignals(input, haystack) {
  ...
  return {
    ...
    sequencingChanged: supplied.sequencingChanged === true,
    evidenceGapResolved: supplied.evidenceGapResolved === true,
    painScore,
    domsScore,
    rpe,
...
export function evaluateMateriality(input = {}) {
  ...
  if (signals.sequencingChanged) {
    escalate('RECOMPUTE_RECOMMENDATION', 'SEQUENCING_CHANGED', ['TRAINING_RESPONSE','NEXT_SESSION']);
  }
  if (signals.evidenceGapResolved) {
    escalate('RECOMPUTE_RECOMMENDATION', 'DECISION_EVIDENCE_GAP_RESOLVED', ['LOAD','NEXT_SESSION']);
  }
