...
export const SESSION_PROTOCOL_FAMILIES = Object.freeze({
  ...
  TARGETED_QUALITY_TRAINING_ONLY: family('TARGETED_QUALITY_TRAINING_ONLY',{
    lane:'ADAPT',optionKeys:['targeted-quality'],prescriptionLevel:'STRUCTURED',defaultComparisonClass:'TRAINING_ONLY',
    capabilityQuestion:'Can a deliberate quality exposure be prescribed safely when the exact repeatable measurement family is not yet established?',
    benchmarkInvariants:[],
    comparisonMetrics:['sessionRpe','completionQuality','limitingSystem'],
    requiredCapture:['sessionRpe','completionQuality','limitingSystem','executionDeviation'],
    notes:['This is an explicit non-benchmark fallback. It must not be presented as longitudinally comparable until promoted into a named protocol family.']
  })
});
...
export function protocolFamilyForOption({lane,optionKey,targetedGaps=[]}={}){
  if(lane==='ADAPT'&&optionKey==='station-work-rate'&&targetedGaps.includes('strength_endurance.repeatability')){
    return SESSION_PROTOCOL_FAMILIES.STRENGTH_ENDURANCE_REPEATABILITY;
  }
  if(lane==='ADAPT'&&optionKey==='station-work-rate')return SESSION_PROTOCOL_FAMILIES.STATION_WORK_RATE;
  return protocolFamilyForOptionKey(optionKey);
}
