...
async function supersedePriorOpenPlans({plannedForDate,newPlanSessionId,selectedAt}){
  const sql=await getSql();
  const rows=await sql`
    ...
    WHERE s.local_date=${plannedForDate}::date
      AND s.session_id<>${newPlanSessionId}
      AND s.status IN ('PLANNED','RECOMMENDED','ACCEPTED','MODIFIED')
      AND r.source_key=${SOURCE_KEY}
      AND r.record_type='planned_workout'
      AND r.payload->>'contextType'=${PLAN_CONTEXT}
      AND NOT EXISTS (
        SELECT 1 FROM fz_training_session_sources x
        WHERE x.session_id=s.session_id AND x.relationship='EXECUTION'
      )
    ORDER BY s.session_id
  `;
  const superseded=[];
  for(const row of rows||[]){
    const priorSessionId=row.session_id;
    await setTrainingSessionState(priorSessionId,{status:'SUPERSEDED',parentSessionId:newPlanSessionId});
    await appendAthleteEvent({
      eventKey:`supersede:fz-choice:${priorSessionId}:${newPlanSessionId}`,
      sessionId:priorSessionId,
      eventType:'LINKED',
      occurredAt:selectedAt,
      localDate:plannedForDate,
      actor:'SYSTEM',
      sourceKey:SOURCE_KEY,
      certainty:'INFERRED',
      summary:'Prior open athlete-selected plan superseded by a later choice for the same planned date',
      payload:{canonicalReplacementSessionId:newPlanSessionId,reason:'LATER_ATHLETE_CHOICE_SAME_DATE'}
    });
...
function normalizeChoice(input,activeRow){
  ...
  if(option.prescription?.selectionReady===false)throw new Error('choice_prescription_not_ready');
  if(['MAINTAIN','ADAPT'].includes(lane)&&(!option.prescription?.protocolFamilyId||!option.prescription?.prescriptionFingerprint))throw new Error('choice_prescription_required');
...
function decisionIdentity(choice){
  return `decision:4.4:${digest({recommendationVersion:choice.recommendationVersion,lane:choice.lane,optionId:choice.optionId,prescriptionFingerprint:choice.option?.prescription?.prescriptionFingerprint||null,plannedForDate:choice.plannedForDate,plannedStartAt:choice.plannedStartAt,overrideReason:choice.overrideReason}).slice(0,20)}`;
}
...
export async function recordAthleteChoice(input={}){
  ...
  const decisionPayload={
    ...
    protocolFamilyId:prescription?.protocolFamilyId||null,protocolVersion:prescription?.protocolVersion||null,
    comparisonClass:prescription?.comparisonClass||'TRAINING_ONLY',prescriptionFingerprint:prescription?.prescriptionFingerprint||null,
    matchesFzRecommendation:choice.matchesFzRecommendation,
    overrideReason:choice.overrideReason,
    selectedAt,selectedLocalDate:decisionLocalDate,plannedForDate:choice.plannedForDate,plannedStartAt:choice.plannedStartAt,
    ...
    rules:{doesNotRewriteRecommendation:true,safetyOverrideCannotBeBypassed:true,selectedPrescriptionImmutable:true}
  };
  const decisionRecord=await ingestTrainingSourceRecord({
    sourceKey:SOURCE_KEY,recordType:'decision',sourceRecordId:decisionId,
    sourceUpdatedAt:selectedAt,localDate:decisionLocalDate,payload:decisionPayload
  });
  const planPayload={
    ...
    prescription,
    protocolFamilyId:prescription?.protocolFamilyId||null,protocolVersion:prescription?.protocolVersion||null,
    prescriptionLevel:prescription?.prescriptionLevel||null,comparisonClass:prescription?.comparisonClass||'TRAINING_ONLY',
    prescriptionFingerprint:prescription?.prescriptionFingerprint||null,benchmarkInvariants:prescription?.benchmarkInvariants||[],declaredVariantChanges:prescription?.declaredVariantChanges||[],
    comparisonMetrics:prescription?.comparisonMetrics||[],requiredCapture:prescription?.requiredCapture||[],
    provenance:{decisionRecordPk:decisionRecord.id,recommendationVersion:choice.recommendationVersion,composerVersion:choice.option.composerVersion||choice.active.sessionOptionComposerVersion||null,prescriptionComposerVersion:choice.option.sessionPrescriptionComposerVersion||choice.active.sessionPrescriptionComposerVersion||null},
    rules:{exactSelectedPrescriptionImmutable:true,comparisonClassMustGovernLongitudinalUse:true}
  };
  ...
  const supersededPlanIds=await supersedePriorOpenPlans({plannedForDate:choice.plannedForDate,newPlanSessionId:planSessionId,selectedAt});
  ...
  const eventPayload={
    ...
    protocolFamilyId:prescription?.protocolFamilyId||null,protocolVersion:prescription?.protocolVersion||null,comparisonClass:prescription?.comparisonClass||'TRAINING_ONLY',prescriptionFingerprint:prescription?.prescriptionFingerprint||null,
    matchesFzRecommendation:choice.matchesFzRecommendation,overrideReason:choice.overrideReason,
    supersededPlanIds,
    sourceOrigin:'adaptive-choice-v4.6-prep',provenance:{decisionRecordPk:decisionRecord.id,planRecordPk:planRecord.id}
  };
  const insertedEvent=await appendAthleteEvent({
    eventKey,sessionId:planSessionId,eventType,occurredAt:selectedAt,localDate:decisionLocalDate,
  ...
  const propagation=await propagateCanonicalChangeSafely({
    ...
    trigger:{type:'ATHLETE_CHOICE',decisionId,evidenceKey:eventKey,materialityLevel:assessment.level,reasonCodes:assessment.reasonCodes,supersededPlanIds},
  ...
  return {
    ...
    protocolFamilyId:prescription?.protocolFamilyId||null,protocolVersion:prescription?.protocolVersion||null,comparisonClass:prescription?.comparisonClass||'TRAINING_ONLY',prescriptionFingerprint:prescription?.prescriptionFingerprint||null,
    selectedLocalDate:decisionLocalDate,plannedForDate:choice.plannedForDate,supersededPlanIds,propagation
  };
}
