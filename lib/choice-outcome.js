...
function executionSnapshot(session={},previous={}){
  const metrics=session.metrics||{};
  return {
    status:upper(session.status)||previous.status||null,
    modality:session.classification?.modality||session.sport_type||previous.modality||null,
    sessionKind:session.session_kind||previous.sessionKind||null,
    title:session.title||session.source_title||previous.title||null,
    actualStartAt:session.actual_start_at||previous.actualStartAt||null,
    durationSeconds:metrics.durationSeconds??previous.durationSeconds??null,
    distanceMeters:metrics.distanceMeters??previous.distanceMeters??null,
    avgHeartRate:metrics.avgHeartRate??previous.avgHeartRate??null,
    maxHeartRate:metrics.maxHeartRate??previous.maxHeartRate??null,
    calories:metrics.calories??previous.calories??null,
    avgPowerWatts:metrics.avgPowerWatts??previous.avgPowerWatts??null,
    paceSecPerKm:metrics.paceSecPerKm??previous.paceSecPerKm??null,
    cadence:metrics.cadence??previous.cadence??null
  };
}
...
export function buildChoiceOutcomeObservation({plan,execution,matchConfidence=null,matchReasons=[],athleteEvent=null,materiality=null,previous=null}={}){
  ...
  return {
    ...
    expectedCost,
    execution:{...executionSnapshot(execution,previous?.execution||{}),matchConfidence:matchConfidence??previous?.execution?.matchConfidence??null,matchReasons:unique(matchReasons?.length?matchReasons:(previous?.execution?.matchReasons||[]))},
    response,
