...
export async function recordExerciseAthleteResponse(input = {}) {
  ...
  // Current Athlete State is a durable projection over Athlete Memory. Recompute it
  // before downstream propagation so readiness/adaptive context consume present-tense
  // truth rather than whichever Athlete Voice event happened to be newest.
  const currentState=await recomputeCurrentAthleteStateSafely({now:new Date(normalized.reportedAt),persist:true});

  // Evidence, event materiality and current athlete state are durable before downstream propagation runs.
  ...
  const propagation = await propagateCanonicalChangeSafely({
    changedNodes:[
      'source.athlete.feedback',
      'athlete.memory',
      'materiality.current',
      ...(currentState?.changed===true?['athlete.state.current']:[]),
      ...(choiceOutcome?.observation?['choice.outcome']:[])
    ],
    materiality,
    trigger:{
      ...
      currentAthleteStateChanged:currentState?.changed===true,
      currentAthleteStateStatus:currentState?.status||null
    },
  ...
  return {
    ...
    currentAthleteState:currentState?.payload||null,
    currentAthleteStateError:currentState?.status==='ERROR'?currentState.error:null,
