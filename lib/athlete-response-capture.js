...
export async function recordExerciseAthleteResponse(input = {}) {
  ...
  const materiality = evaluateMateriality({sourceType:'ATHLETE_FEEDBACK',eventType:result.eventType,certainty:result.certainty,categories:result.categories,summary:text(normalized.summary||normalized.rawText),rawText:text(normalized.rawText),occurredAt:normalized.occurredAt,reportedAt:normalized.reportedAt,sessionStatus:result.resolution?.linkedSession?.status||null,signals:normalized.materialitySignals||normalized.payload?.materialitySignals||{},payload:normalized.payload});
  const materialityRecord = await persistMaterialityAssessment({evidenceKey:result.eventKey,sourceType:'ATHLETE_FEEDBACK',sourceKey:normalized.ingestSourceKey,sourceRecordPk:result.sourceRecordPk,athleteEventId:result.eventId,occurredAt:normalized.occurredAt,summary:text(normalized.summary||normalized.rawText),assessment:materiality});
  let recommendationShadow=null;
  if(materiality.shouldRecomputeRecommendation){recommendationShadow=await recomputeRecommendationShadowSafely({materiality,trigger:{type:'ATHLETE_FEEDBACK',evidenceKey:result.eventKey,materialityLevel:materiality.level,reasonCodes:materiality.reasonCodes},now:new Date(normalized.reportedAt),persist:true});}
  return {...result,projectScope:EXERCISE_PROJECT_SCOPE,athleteId:FZ_ATHLETE_ID,speakerResolution:normalized.speakerResolution,reportedAt:normalized.reportedAt,occurredAt:normalized.occurredAt,occurrencePrecision:normalized.timestampPrecision,materiality:{...materiality,persistedRecordPk:materialityRecord.id},recommendationShadow};
}
