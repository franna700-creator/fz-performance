...
const HYBRID_PROTOCOL_FAMILIES = new Set(['COMPROMISED_RUNNING_REPEATABILITY','HYBRID_TRANSITION_ECONOMY','STATION_WORK_RATE','STRENGTH_ENDURANCE_REPEATABILITY']);
...
function protocolFamilies(sources = []) {
  return unique(sources.filter(source=>source?.relationship==='PLAN'&&source?.record_type==='planned_workout').map(source=>source?.payload?.protocolFamilyId||source?.payload?.prescription?.protocolFamilyId));
}
...
function sourceModality(session = {}, sources = []) {
  const families=protocolFamilies(sources);
  if(upper(session.session_kind)==='HYROX'||families.some(id=>HYBRID_PROTOCOL_FAMILIES.has(id))) return {code:'HYROX_MIXED',label:'HYROX mixed',strength:4,reason:families.length?`canonical protocol family ${families.join(', ')}`:'canonical HYROX session kind'};
