...
function appendCurrentSeriesPoint(rows, name, observedAt, value) {
  if (!observedAt || !validSeriesValue(name, value)) return;
  if (rows.some(row => row.series_name === name && row.observed_at === observedAt)) return;
  rows.push({
...
function validSleepScore(value) {
...
export function normalizeGarminCiqPayload(raw = {}) {
  ...
  const sleepScore = number(current.sleepScore ?? current.sleep_score);
  const steps = number(current.steps);

  appendCurrentSeriesPoint(series, 'heart_rate', observedAt, heartRate);
  appendCurrentSeriesPoint(series, 'stress', observedAt, stress);
  appendCurrentSeriesPoint(series, 'body_battery', observedAt, bodyBattery);
  appendCurrentSeriesPoint(series, 'respiration', observedAt, respiration);
  series.sort((a, b) => Date.parse(a.observed_at) - Date.parse(b.observed_at));
  ...
  return {
    snapshot: {
      ...
      hrvLastNight: null,
      sleepScore: validSleepScore(sleepScore) ? sleepScore : null,
      sleepHours: null,
