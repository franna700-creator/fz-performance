...
export async function buildAdaptiveContext(options={}){
  ...
  const asOf=baseContext.asOf||localDate(nowDate);
  ...
  if(sequencing){
    ...
    context={
      ...baseContext,
      sequencing:{...(baseContext.sequencing||{}),...sequencing},
  ...
  return mergeCanonicalReadinessIntoContext(context,readinessRow?.payload||null);
}
