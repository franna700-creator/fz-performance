...
const SOURCE_PRIORITY = [INTERVALS_ICU_SOURCE_KEY, FITNESS_AI_SOURCE_KEY];
...
export async function getWellnessToday(date = localDateSast(), { sourceKey = null } = {}) {
  ...
  const snapshotRows = sourceKey
    ...
    : await sql`
        ...
        ORDER BY
          CASE
            WHEN source_key = ${SOURCE_PRIORITY[0]} THEN 0
            WHEN source_key = ${SOURCE_PRIORITY[1]} THEN 1
            ELSE 2
          END,
          source_as_of DESC, ingested_at DESC, id DESC
        LIMIT 1
      `;
  ...
  const seriesRows = await sql`
    ...
    WHERE source_key = ${row.source_key} AND local_date = ${date}::date
