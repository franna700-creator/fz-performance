...
export async function recordAthleteMemory(input = {}) {
  ...
  const result = await sql`
    SELECT fz_record_athlete_memory(
      ${eventKey}, ${eventType}, ${occurredAt.toISOString()}::timestamptz, ${localDate}::date,
      ${summary}, ARRAY(SELECT jsonb_array_elements_text(${JSON.stringify(categories)}::jsonb)),
      ${resolution.session?.session_id || null}, ${certainty}, ${JSON.stringify(payload)}::jsonb,
      ${text(input.ingestSourceKey) || 'conversation'}, ${eventKey}, ${resolution.method},
      ${resolution.session ? resolution.confidence : null}
    ) AS result
  `;
  return {
    ...result[0].result,
