...
import { createCanonicalRevision, markConvergenceNode, markConvergenceNodes, readConvergenceStatus, reconcileSupersededOpenRevisions } from './convergence-store.js';
import { acknowledgeCoveredCanonicalMutations } from './canonical-mutation-outbox.js';
...
const ADAPTIVE_CONTEXT_DERIVATIONS = new Set([
  ...
  'event.demand_taxonomy','event.intelligence','capability.evidence','capability.priority','measurement.evidence','measurement.hierarchy','adaptive.context'
]);
...
export async function propagateCanonicalChange({
  ...
  const temporalInvalidation = trigger?.temporalInvalidation === true;
  const shouldRecompute = recommendationAffected && (
    forceRecommendationRecompute || temporalInvalidation || readinessChanged || materiality?.shouldRecomputeRecommendation === true ||
    ...
    if (shouldRecompute) {
      ...
      if (shadow?.status === 'ERROR') {
        ...
        await mark(revision.revisionId,'recommendation.shadow','RECOMPUTED',{
          ...
          trigger:{forceRecommendationRecompute,temporalInvalidation,readinessChanged,materialityRecompute:materiality?.shouldRecomputeRecommendation===true,fingerprintChanged:!!fingerprintChanged}
        },warnings);
      }
  ...
  let outboxAcknowledgement=null;
  try{
    outboxAcknowledgement=await acknowledgeCoveredCanonicalMutations({dependencyClosure:closure,revisionId:revision.revisionId,occurredAt:revision.occurredAt});
  }catch(error){
    warnings.push(`Canonical mutation outbox acknowledgement: ${errorText(error)}`);
  }

  const convergence = await readConvergenceStatus(revision.revisionId);
  let supersession={resolvedRows:0,resolvedRevisions:[]};
  if(!convergence.pending){
    try{
      supersession=await reconcileSupersededOpenRevisions({revisionId:revision.revisionId});
    }catch(error){
      warnings.push(`Convergence supersession reconciliation: ${errorText(error)}`);
    }
  }
  current = await readIntelligenceCurrent({ now });
  return {
