...
const ADAPTIVE_CONTEXT_DERIVATIONS = new Set([
  'recovery.current','training.identity','training.intent','trends.ncl','load.rolling','performance.aet',
...
export async function propagateCanonicalChange({
  ...
  for(const node of closure){
    if(node.startsWith('source.')||node.startsWith('process.')){
      await mark(revision.revisionId,node,'RECONCILED',{reason:'SOURCE_OR_RECONCILIATION_COMPLETED_BEFORE_OR_DURING_PROPAGATION'},warnings);
