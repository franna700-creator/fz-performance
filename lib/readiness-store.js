...
export async function readReadinessInputs({ date = localDate() } = {}) {
  ...
  const currentRows = await sql`
    ...
    WHERE local_date=${date}::date
      AND source_key IN ('intervals-icu','fitness-ai')
    ORDER BY CASE source_key
               WHEN 'intervals-icu' THEN 0
               WHEN 'fitness-ai' THEN 1
               ELSE 2
             END,
             source_as_of DESC NULLS LAST,ingested_at DESC,id DESC
    LIMIT 1
  `;
  ...
  const [historyRows, athleteStateRow, ciqRows] = await Promise.all([
    sql`
  ...
  const ciqRow = ciqRows?.[0] || null;
  const dailyCurrent = wellnessShape(currentRow);
  const ciqCurrent = wellnessShape(ciqRow || {});
  const ciqSleepScoreUsed = dailyCurrent.sleepScore == null && ciqCurrent.sleepScore != null;
  const current = {
    ...dailyCurrent,
    sleepScore: ciqSleepScoreUsed ? ciqCurrent.sleepScore : dailyCurrent.sleepScore
  };
  return {
    date,
    currentRow,
    ciqFallbackRow: ciqSleepScoreUsed ? ciqRow : null,
    current,
    history,
    ...
    sourceAsOf: maxIso(
      ...
      ciqSleepScoreUsed ? ciqRow?.source_as_of : null,
...
export function buildReadinessCandidate(inputs = {}) {
  ...
  const fingerprintBasis = {
    ...
    wellnessFallback: inputs.ciqFallbackRow ? {
      sourceKey: inputs.ciqFallbackRow.source_key,
      sourceAsOf: inputs.ciqFallbackRow.source_as_of || null,
      sourceHash: inputs.ciqFallbackRow.source_hash || null,
      fields: ['sleepScore']
    } : null,
  ...
  return {
    ...
    evidence: {
      ...
      wellnessFallback: inputs.ciqFallbackRow ? {
        sourceKey: inputs.ciqFallbackRow.source_key,
        snapshotId: Number(inputs.ciqFallbackRow.id),
        sourceAsOf: inputs.ciqFallbackRow.source_as_of || null,
        sourceHash: inputs.ciqFallbackRow.source_hash || null,
        fields: ['sleepScore']
      } : null,
