...
export const MEASUREMENT_EVIDENCE_VERSION = '1.0.1';
...
const withinWindow = (value, asOf) => {
  const date = localDateText(value);
  if (!date) return false;
  if (!asOf) return true;
  const ageDays = (Date.parse(`${asOf}T12:00:00Z`) - Date.parse(`${date}T12:00:00Z`)) / 86400000;
  return date <= asOf && ageDays >= 0 && ageDays < MEASUREMENT_LOOKBACK_DAYS;
};
...
export function deriveMeasurementEvidence({range={},trends=null,asOf=null}={}) {
  ...
  for (const session of [...(range.sessions || [])].sort((a,b)=>String(a.session_id).localeCompare(String(b.session_id)))) {
    const observedOn = localDateText(session.local_date);
    if (!['COMPLETED','STOPPED_EARLY'].includes(session.status) || !session.actual_start_at || !withinWindow(session.local_date,asOf)) continue;
    ...
    const observation = {
      sessionId:session.session_id,observedOn,protocolFamilyId:familyId,
  ...
  for (const row of trends?.performance?.matchedAet || []) {
    ...
    const observation={kind:'MATCHED_AET',sessionId:row.sessionId || row.date,observedOn:localDateText(observedOn),sourceRefs:[`matched-aet:${row.sessionId || row.date}`],metrics:row};
