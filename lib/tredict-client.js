...
async function activityDetail(args = {}) {
  ...
  // Tredict only includes intensityDistribution / zonesDistribution when
  // extraValues=1 is requested. NCL is derived from that canonical HR-zone
  // evidence, so detail ingestion must always request the enriched contract.
  return requestJson(`activity/${id}`, { extraValues: 1 });
}
