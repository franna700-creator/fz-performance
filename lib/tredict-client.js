...
export function tredictNextCursor(data = {}) {
  const cursor = data?.pagination?.nextCursor ?? data?._pagination?.nextCursor ?? data?.page?.nextCursor ?? null;
...
async function activityList(args = {}) {
  ...
  const maxPages = Math.min(100, Math.max(1, Number(args.maxPages) || 25));
  const requestedEndDate = args.endDate ? String(args.endDate) : null;
  let pageStartDate = args.startDate ? String(args.startDate) : null;
  ...
  for (let page = 0; page < maxPages; page += 1) {
    ...
    for (const row of activityRows(data)) {
      const flat = flattenActivity(row);
      const id = String(flat?.id ?? flat?._id ?? '').trim();
      const key = id || `${flat?.date || ''}:${flat?.sportType || ''}:${byId.size}`;
      if (!byId.has(key)) byId.set(key, flat);
    }

    const nextCursor = tredictNextCursor(data);
    if (!nextCursor) return [...byId.values()];
    if (requestedEndDate) {
      const cursorMs = Date.parse(nextCursor);
      const endMs = Date.parse(requestedEndDate);
      if (Number.isFinite(cursorMs) && Number.isFinite(endMs) && cursorMs < endMs) return [...byId.values()];
    }
    if (seenCursors.has(nextCursor) || nextCursor === pageStartDate) {
      throw new Error('Tredict activity pagination stalled on a repeated cursor');
    }
    seenCursors.add(nextCursor);
    pageStartDate = nextCursor;
  }

  throw new Error(`Tredict activity pagination exceeded ${maxPages} pages before range completion`);
}
