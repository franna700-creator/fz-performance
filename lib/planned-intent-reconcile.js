...
import { persistChoiceOutcome } from './choice-outcome-store.js';
...
export function scoreFzPlannedIntentCandidate(plan,session){
...
export function selectFzPlannedIntentMatch(plan,sessions=[]){
  const candidates=(sessions||[])
    .map(session=>({session,...(scoreFzPlannedIntentCandidate(plan,session)||{score:-1,reasons:[]})}))
    .filter(row=>row.score>=0)
    .sort((a,b)=>b.score-a.score||String(a.session?.session_id||'').localeCompare(String(b.session?.session_id||'')));
  const best=candidates[0]||null,second=candidates[1]||null;
  if(best&&best.score>=.68&&(!second||best.score-second.score>=.12))return {state:'MATCHED',best,second,candidates};
  if(best&&best.score>=.55)return {state:'AMBIGUOUS',best,second,candidates};
  return {state:'NO_CANDIDATE',best,second,candidates};
}
...
export async function reconcileFzPlannedIntents({startDate,endDate}={}){
  ...
  for(const plan of plans){
    const selection=selectFzPlannedIntentMatch(plan,executions);
    if(selection.state!=='MATCHED'){
      if(selection.state==='AMBIGUOUS'){ambiguous+=1;await setTrainingSessionState(plan.session_id,{reconciliationState:'MATCH_REQUIRED'});}
      else noCandidate+=1;
      continue;
    }
    const best=selection.best,actual=best.session;
    ...
    let outcome=null,outcomeError=null;
    try{outcome=await persistChoiceOutcome({plan,execution:actual,matchConfidence:Number(best.score.toFixed(2)),matchReasons:best.reasons});}
    catch(error){outcomeError=error instanceof Error?error.message:String(error);}
    matched+=1;changes.push({eventKey,plannedSessionId:plan.session_id,sessionId:actual.session_id,decisionId:plan.payload?.decisionId||null,optionId:plan.payload?.optionId||null,confidence:Number(best.score.toFixed(2)),reasons:best.reasons,outcomeState:outcome?.status||null,outcomeRecordPk:outcome?.row?.id||null,outcomeError});
  }
