...
import { attachSessionPrescriptions, SESSION_PRESCRIPTION_COMPOSER_VERSION } from './session-prescription-composer.js';
...
export function buildActiveRecommendationPayload(shadowPayload = {}) {
  ...
  const composed = status === 'READY' ? composeSessionOptions(shadowPayload) : { ABSORB: [], MAINTAIN: [], ADAPT: [] };
  const lanes = status === 'READY' ? attachSessionPrescriptions(composed, shadowPayload) : composed;
  ...
  return {
    ...
    rules: {
      ...
      sessionOptionComposerVersion: SESSION_OPTION_COMPOSER_VERSION,
      sessionPrescriptionComposerVersion: SESSION_PRESCRIPTION_COMPOSER_VERSION,
      nonReadyPrescriptionCannotBeSelected: true
    }
  };
}
