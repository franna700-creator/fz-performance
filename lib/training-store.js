  ...
  const rows = await sql`
    ...
    ON CONFLICT (session_id) DO UPDATE SET
      ...
      reconciliation_state = CASE
        -- Source ingestion is not an explicit reconciliation action. It may improve
        -- canonical reconciliation confidence, but it may not silently downgrade it.
        WHEN fz_training_sessions.reconciliation_state='MANUAL' THEN 'MANUAL'
        WHEN fz_training_sessions.reconciliation_state='MATCHED'
          AND EXCLUDED.reconciliation_state IN ('UNMATCHED','MATCH_REQUIRED') THEN 'MATCHED'
        WHEN fz_training_sessions.reconciliation_state='MATCH_REQUIRED'
          AND EXCLUDED.reconciliation_state='UNMATCHED' THEN 'MATCH_REQUIRED'
...
export async function findSessionBySourceRecord(sourceRecordPk) {
  const rows = await findSessionsBySourceRecord(sourceRecordPk);
  return rows[0] || null;
}

export async function findSessionsBySourceRecord(sourceRecordPk) {
  const sql = await getSql();
  return sql`
    SELECT s.*, l.relationship, l.match_method, l.match_confidence, l.linked_at
    ...
    ORDER BY l.linked_at DESC, s.updated_at DESC
  `;
}
