...
import { deriveMeasurementEvidence, MEASUREMENT_LOOKBACK_DAYS } from './measurement-evidence.js';
...
function wellnessSummary(asOf,wellness){
  const current=wellness?.current||{};
  const parts=[];
  const sleepHours=n(current.sleepHours),sleepScore=n(current.sleepScore),rhr=n(current.restingHeartRate),hrv=n(current.hrv),bbHigh=n(current.bodyBatteryHigh);
  if(sleepHours!==null||sleepScore!==null){
    const sleep=[];
    if(sleepHours!==null)sleep.push(`${Math.round(sleepHours*100)/100} h`);
    if(sleepScore!==null)sleep.push(`score ${Math.round(sleepScore)}`);
    parts.push(`sleep ${sleep.join(' / ')}`);
  }
  if(rhr!==null)parts.push(`RHR ${Math.round(rhr)}`);
  if(hrv!==null)parts.push(`HRV ${Math.round(hrv)}`);
  if(bbHigh!==null)parts.push(`Body Battery high ${Math.round(bbHigh)}`);
  return parts.length?`Current ${asOf} wellness: ${parts.join(', ')}.`:null;
}
export function resolveCurrentRecovery({asOf,runtimeStateId,readiness={},wellness=null,materiality=null}={}){
  const runtimeReadinessDate=dateOnly(runtimeStateId);
  const runtimeReadinessFresh=Boolean(asOf&&runtimeReadinessDate===asOf);
  const currentReadiness=runtimeReadinessFresh?(readiness||{}):{};
  const reasonCodes=materiality?.reasonCodes||[];
  const wellnessDeterioration=reasonCodes.includes('WELLNESS_MULTI_SIGNAL_DETERIORATION');
  const systemicState=currentReadiness.systemicRecovery||wellnessSummary(asOf,wellness);
  const status=currentReadiness.status||(wellnessDeterioration?'CURRENT WELLNESS · RECOVERY CONSTRAINED':'CURRENT READINESS SCORE UNAVAILABLE');
  const localConstraint=currentReadiness.localTissueState||null;
  return {
    freshnessPolicyVersion:CURRENT_RECOVERY_FRESHNESS_POLICY,
    runtimeReadinessFresh,
    runtimeReadinessDate,
    readinessScore:n(currentReadiness.score),
    status,
    systemicState,
    localConstraint,
    recoveryText:`${status||''} ${systemicState||''} ${localConstraint||''}`.trim()
  };
}
export function deriveConstraintSeverity(materiality,recoveryText=''){
  ...
  if(codes.some(code=>/PAIN_HIGH|DOMS_HIGH|GI_LIMITER_HIGH|CURRENT_CONSTRAINT_HIGH|ATHLETE_RESPONSE_MATERIAL_NEGATIVE|WELLNESS_MULTI_SIGNAL_DETERIORATION/.test(code)))return 'HIGH';
...
export async function buildAdaptiveContext({now=new Date(),materialityOverride=null,trigger={}}={}){
  ...
  const [runtime,wellness,trends,range,materialityRows]=await Promise.all([
    ...
    readTrainingRange(addDays(asOf,-(MEASUREMENT_LOOKBACK_DAYS-1)),addDays(asOf,2)).catch(()=>({sessions:[],events:[],sources:[],unavailable:true})),
  ...
  const recovery=resolveCurrentRecovery({asOf,runtimeStateId:runtime?.stateId||runtimeState?.stateId||runtimeState?.candidateStateId||runtimeState?.masterAsOf,readiness,wellness,materiality});
  const recoveryText=recovery.recoveryText;
  const measurementDerivation=deriveMeasurementEvidence({trends,range,asOf});
  const mEvidence=measurementDerivation.evidence;
  const measurement=resolvePrimaryObjectiveMeasurementHierarchy({objectiveContext:eventIntelligence,formatRegistry:objectiveRuntime.formatRegistry,measurementRegistry,objectiveRegistry:objectiveRuntime.objectiveRegistry,evidence:mEvidence});
  if(range.unavailable)measurement.status='EVIDENCE_UNAVAILABLE';
  ...
  const readinessScore=recovery.readinessScore;
  ...
  if(range.unavailable)missing.push('canonical measurement evidence unavailable');
  const evidenceRows=[];
  ...
  else if(recovery.systemicState)evidenceRows.push(evidence(`wellness:${asOf}:current`,recovery.systemicState,'Neon current wellness','DIRECT'));
  ...
  return {
    ...
    capabilityEvidence:(objectiveRuntime.objectiveRegistry.capabilities||[]).filter(cap=>(eventIntelligence.primaryEvent?.demands||[]).some(d=>d.capabilityId===cap.id)).map(cap=>{
      const observations=mEvidence[`CAPABILITY:${cap.id}`]?.observations||[];
      const sessionCount=new Set(observations.map(row=>row.sessionId)).size;
      return {id:cap.id,name:cap.name,status:range.unavailable?'UNAVAILABLE':sessionCount?'EVIDENCE AVAILABLE':'UNMEASURED',
        summary:range.unavailable?'Canonical measurement evidence is temporarily unavailable.':sessionCount?'Qualified protocol evidence is available. Capability improvement and event attainment require separate longitudinal interpretation.':'No qualified protocol evidence in the current measurement window; this is not evidence of weakness.',
        evidence:sessionCount?`${sessionCount} canonical session(s)`:'Not yet measured',
        next:sessionCount?'Compare qualified executions and their recovery cost.':'Capture an objective-relevant qualified execution.',
        observations,asOf,lookbackDays:MEASUREMENT_LOOKBACK_DAYS};
    }),
    recovery:{
      freshnessPolicyVersion:recovery.freshnessPolicyVersion,
      runtimeReadinessFresh:recovery.runtimeReadinessFresh,
      runtimeReadinessDate:recovery.runtimeReadinessDate,
      readinessScore,
      status:recovery.status,
      systemicState:recovery.systemicState,
      localConstraint:recovery.localConstraint,
      constraintSeverity:deriveConstraintSeverity(materiality,recoveryText),
      safetyBlock:materiality?.blocksExistingRecommendation===true,
      wellnessFreshness:wellness?.freshness||null
    },
    ...
    measurement:{evidenceVersion:measurementDerivation.version,evidenceFingerprint:measurementDerivation.fingerprint,coverage:measurementDerivation.coverage,diagnostics:measurementDerivation.diagnostics,gaps:measurement.gaps||[],status:measurement.status,hierarchyId:measurement.hierarchyId||null,topGaps,measured:(measurement.measurements||[]).filter(x=>x.evidence?.status==='MEASURED').map(x=>({measurementId:x.id,priority:x.priority,evidence:x.evidence}))},
    materiality,
    ...
    rules:{primaryObjectiveCannotBeDisplacedByProximity:true,unknownMeasurementIsNotWeakness:true,qualifiedEventStructureRequired:true,eventDataIsRuntimeVariable:true,staticEventSeedCannotOverrideRuntime:true,shadowRecommendationMayNotAlterToday:true,staleRuntimeReadinessCannotRepresentCurrentState:true}
  };
}
