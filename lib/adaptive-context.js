...
function constraintSeverity(materiality,recoveryText=''){
  ...
  if(/not fully recovered|heavy legs|severe|pain|illness|limiter/.test(recoveryText.toLowerCase()))return 'MODERATE';
  return 'NONE';
}
...
export async function buildAdaptiveContext({now=new Date(),materialityOverride=null,trigger={}}={}){
  ...
  const primaryKnowledge=eventIntelligence.eventKnowledge.find(x=>x.eventId===eventIntelligence.primaryEvent?.id)||null;
  const primaryObjective=eventIntelligence.primaryEvent?{
    id:eventIntelligence.primaryEvent.id,name:eventIntelligence.primaryEvent.name,role:eventIntelligence.primaryEvent.role,
    runwayDays:eventIntelligence.primaryEvent.runwayDays,knowledgeStatus:primaryKnowledge?.knowledgeStatus||'PENDING_RESEARCH',formatProfileId:eventIntelligence.primaryEvent.formatProfileId||null
  }:null;
  const eventPressure=(eventIntelligence.eventKnowledge||[]).filter(x=>x.eventId!==primaryObjective?.id).map(x=>({id:x.eventId,name:x.eventName,role:x.role,runwayDays:x.runwayDays,knowledgeStatus:x.knowledgeStatus,overlapToPrimary:x.overlapToPrimary?.score??null}));
  ...
  if(objectiveRuntime.source!=='NEON_OBJECTIVE_GRAPH'){missing.push('runtime objective graph');assumptions.push('Versioned config objective seed is used for shadow evaluation only.');}
  if(!materiality)missing.push('recent materiality assessment');
  const evidenceRows=[];
  if(primaryObjective)evidenceRows.push(evidence(`objective:${primaryObjective.id}`,`${primaryObjective.name} is the resolved ${primaryObjective.role} objective with ${primaryObjective.runwayDays} day(s) runway.`,objectiveRuntime.source,'DIRECT'));
  ...
  return {
    ...
    recovery:{readinessScore,status:readiness.status||null,systemicState:readiness.systemicRecovery||null,localConstraint:readiness.localTissueState||null,constraintSeverity:constraintSeverity(materiality,recoveryText),safetyBlock:materiality?.blocksExistingRecommendation===true,wellnessFreshness:wellness?.freshness||null},
    ...
    rules:{primaryObjectiveCannotBeDisplacedByProximity:true,unknownMeasurementIsNotWeakness:true,qualifiedEventStructureRequired:true,shadowRecommendationMayNotAlterToday:true}
  };
}
