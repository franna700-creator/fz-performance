...
export async function persistChoiceOutcome({plan,execution,matchConfidence=null,matchReasons=[],athleteEvent=null,materiality=null}={}){
  ...
  if(!source.decisionId||!execution?.session_id)return {status:'NOT_APPLICABLE',observation:null,row:null};
  const previous=await readLatestChoiceOutcome(source.decisionId,execution.session_id);
  const observation=buildChoiceOutcomeObservation({plan,execution,matchConfidence,matchReasons,athleteEvent,materiality,previous:previous?.payload||null});
  ...
  return {status:observation.observationState,observation,row};
}
...
export async function persistChoiceOutcomeResponse({sessionId,athleteEvent,materiality}={}){
  ...
  if(!plan)return {status:'NO_FZ_PLAN',observation:null,row:null};
  const sql=await getSql();
  const sessions=await sql`SELECT * FROM fz_training_sessions WHERE session_id=${sessionId} LIMIT 1`;
  const execution=sessions?.[0];
  if(!execution)return {status:'NO_EXECUTION',observation:null,row:null};
