...
export function decorateTrainingRange(range={}){
  ...
  const hydrate=session=>{
    ...
    return {...session,source_title:session.title,source_sport_type:session.sport_type,title:classification.displayTitle||session.title,sport_type:classification.modalityLabel||session.sport_type,classification,metrics:canonicalEvidence.metrics,evidence:canonicalEvidence.evidence,events,sources};
  };
