...
export function usableMeasurementEvidence(value) {
  if (value == null || value === false || value === '') return false;
  if (typeof value === 'number') return Number.isFinite(value);
  if (Array.isArray(value)) return value.length > 0 && value.every(usableMeasurementEvidence);
  if (typeof value === 'object') {
    if (!Object.keys(value).length) return false;
    if (value.status && ['INVALID','PENDING','UNKNOWN','STALE','REJECTED'].includes(value.status)) return false;
    if ('observations' in value) return Array.isArray(value.observations) && value.observations.length > 0 && value.observations.every(usableMeasurementEvidence);
...
function evidenceStatus(measurement, evidence = {}) {
  ...
  const observed=records.filter(key=>usableMeasurementEvidence(evidence[key]));
  if(!observed.length) return {status:'UNKNOWN',quality:'NONE',evidenceKeys:[]};
  const preferred=(measurement.preferredEvidence||[]).filter(key=>usableMeasurementEvidence(evidence[key]));
  return {status:'MEASURED',quality:preferred.length?'DIRECT_OR_PREFERRED':'PROXY',evidenceKeys:observed,observations:observed.flatMap(key=>(evidence[key]?.observations||[]).map(observation=>({evidenceKey:key,...observation})))};
}
