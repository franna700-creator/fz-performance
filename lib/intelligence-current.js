...
export function buildIntelligenceCurrent(rows = {}) {
  const athleteEventAt = firstValue(rows.athleteEvent, ['created_at', 'occurred_at']);
  ...
  const markers = {
    ...
    athleteEvent: marker(rows.athleteEvent, 'event_id', ['created_at', 'occurred_at']),
  ...
  // Materiality is event-addressed: a newly persisted canonical athlete event must
  // not remain unassessed. Shadow/active recommendation freshness additionally
  // follows objective, training and wellness evidence because those inputs can
  // change adaptive context without creating a new Athlete Voice event.
  const decisionEvidenceAt = newest(athleteEventAt, trainingEvidenceAt, wellnessEvidenceAt, objectiveAt, materialityAt);

  const pending = {
    materiality: newer(athleteEventAt, materialityAt),
    shadowRecommendation: newer(decisionEvidenceAt, shadowAt),
    activeRecommendation: newer(shadowAt, activeAt)
  };
  ...
  return {
    ...
    markers,
    freshness: {
      canonicalEvidenceAt: newest(athleteEventAt, trainingEvidenceAt, wellnessEvidenceAt, objectiveAt),
      materialityAt,
...
export async function readIntelligenceCurrent() {
  ...
  const [runtime, athleteEvent, trainingEvidence, wellnessEvidence, materiality, shadowRecommendation, activeRecommendation, athleteDecision, objective] = await Promise.all([
    ...
    one(sql, db => db`SELECT event_id,occurred_at,created_at FROM fz_athlete_events ORDER BY event_id DESC LIMIT 1`),
    one(sql, db => db`
