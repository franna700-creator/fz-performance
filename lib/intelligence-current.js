...
export async function readIntelligenceCurrent() {
  ...
  const [runtime, athleteEvent, trainingEvidence, wellnessEvidence, materiality, decisionMateriality, shadowRecommendation, activeRecommendation, athleteDecision, objective] = await Promise.all([
    ...
    one(sql, db => db`
      SELECT id,source_updated_at,ingested_at FROM fz_training_source_latest
      WHERE source_key <> 'fz-intelligence'
        AND record_type IN ('planned_workout','executed_activity')
      ORDER BY source_updated_at DESC NULLS LAST,ingested_at DESC,id DESC LIMIT 1
    `),
    ...
    one(sql, db => db`
      ...
      WHERE source_key='fz-intelligence' AND record_type='event_context'
        ...
        AND payload->>'evidenceKey'=(SELECT event_key FROM fz_athlete_events WHERE actor='ATHLETE' ORDER BY event_id DESC LIMIT 1)
      ORDER BY source_updated_at DESC NULLS LAST,ingested_at DESC,id DESC LIMIT 1
    `),
    one(sql, db => db`
      SELECT id,source_updated_at,ingested_at,payload FROM fz_training_source_latest
