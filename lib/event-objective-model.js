...
const EVENT_STATES = new Set(['CANDIDATE','DORMANT','SCHEDULED','ACTIVE','COMPLETED','CANCELLED']);
...
export function eventDateBounds(event = {}) {
  const exact=dateOnly(event.date);
  if(exact) return {start:exact,end:exact,precision:event.datePrecision||'EXACT',exact:true};
  const start=dateOnly(event.startsOn||event.startDate||event?.metadata?.dateWindow?.start);
  const end=dateOnly(event.endsOn||event.endDate||event?.metadata?.dateWindow?.end);
  const low=start||end||null,high=end||start||null;
  return {start:low,end:high,precision:event.datePrecision||event?.metadata?.datePrecision||(low?'WINDOW':'UNKNOWN'),exact:Boolean(low&&high&&low===high)};
}
...
export function validateObjectiveRegistry(registry = {}) {
  ...
  for (const event of events) {
    ...
    if(!['DORMANT','CANDIDATE'].includes(event?.status)&&!bounds.start&&!bounds.end) errors.push(`event ${event?.id || 'unknown'} requires an exact date or date window unless DORMANT`);
    if(bounds.start&&bounds.end&&dayNumber(bounds.end)<dayNumber(bounds.start)) errors.push(`event ${event?.id || 'unknown'} date window ends before it starts`);
...
export function eventRunwayWindow(event, nowDate) {
  const now=dateOnly(nowDate),bounds=eventDateBounds(event);
  if(!now||(!bounds.start&&!bounds.end)) return {minDays:null,maxDays:null,precision:bounds.precision,exact:false};
  const minDays=dayNumber(bounds.start||bounds.end)-dayNumber(now);
  const maxDays=dayNumber(bounds.end||bounds.start)-dayNumber(now);
  return {minDays,maxDays,precision:bounds.precision,exact:bounds.exact};
}

export function eventRunway(event, nowDate) {
  const window=eventRunwayWindow(event,nowDate);
  return window.minDays;
}
...
export function persistedEventState(status) { return status === 'CANDIDATE' ? 'DORMANT' : status; }

export function eventStrategicWeight(event) {
  if (!event || ['CANDIDATE','DORMANT','COMPLETED','CANCELLED'].includes(event.status)) return 0;
...
export function buildObjectiveContext(registry, nowDate) {
  ...
  const scheduled = (registry.events || []).filter(event => {
    if(['CANDIDATE','DORMANT','COMPLETED','CANCELLED'].includes(event.status)) return false;
    if(event.status==='ACTIVE') return true;
    const window=eventRunwayWindow(event,now);
    return window.maxDays===null || window.maxDays>=0;
  }).map(event => {
    const runwayWindow=eventRunwayWindow(event,now);
    return { ...event, runwayDays:runwayWindow.minDays, runwayWindow, strategicWeightResolved:eventStrategicWeight(event), nearTermPlanningWeight:eventPlanningWeight(event,now) };
  }).sort((a,b)=>(a.runwayDays??Number.POSITIVE_INFINITY)-(b.runwayDays??Number.POSITIVE_INFINITY));
  const primary = scheduled.filter(event => event.role === 'PRIMARY').sort((a,b)=>b.strategicWeight-a.strategicWeight || (a.runwayDays??Number.POSITIVE_INFINITY)-(b.runwayDays??Number.POSITIVE_INFINITY))[0] || null;
  const overlaps=[];
