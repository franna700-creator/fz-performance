export const READINESS_ENGINE_VERSION = '5.0.0-readiness.2';
...
function readableSubject(value='') { return String(value || '').replaceAll('_',' / ').toLowerCase(); }
...
function strongestActiveConstraint(athleteState=null) {
  const rank={SAFETY:4,HIGH:3,MODERATE:2,REPORTED:1};
  const active=Array.isArray(athleteState?.activeConstraints)?athleteState.activeConstraints:[];
  return active.slice().sort((a,b)=>(rank[String(b?.severity||'').toUpperCase()]||0)-(rank[String(a?.severity||'').toUpperCase()]||0)||new Date(b?.lastConfirmedAt||0)-new Date(a?.lastConfirmedAt||0))[0]||null;
}

export function readinessModifierFromAthleteContext({ athleteState = null, athleteEvent = null, materiality = null } = {}) {
  const activeConstraint=strongestActiveConstraint(athleteState);
  if(activeConstraint){
    const severity=String(activeConstraint.severity||'REPORTED').toUpperCase();
    const evidence=[`ACTIVE_CONSTRAINT:${activeConstraint.subject||'UNKNOWN'}:${severity}`];
    const base={subject:activeConstraint.subject||null,severity,evidence};
    if(severity==='SAFETY')return {type:'SAFETY_OVERRIDE',adjustment:0,cap:39,confidenceEffect:'DIRECT_CONSTRAINT',...base};
    if(severity==='HIGH')return {type:'HIGH_CONSTRAINT',adjustment:-8,cap:54,confidenceEffect:'DIRECT_CONSTRAINT',...base};
    if(severity==='MODERATE')return {type:'MODERATE_CONSTRAINT',adjustment:-5,cap:69,confidenceEffect:'DIRECT_CONSTRAINT',...base};
    return {type:'REPORTED_CONSTRAINT',adjustment:0,cap:null,confidenceEffect:'DIRECT_CONSTRAINT',...base};
  }

  const categories = words(athleteEvent?.payload?.memoryCategories || athleteEvent?.categories);
  ...
  if (!athleteEvent || !relevant) {
    return { type: 'NONE', adjustment: 0, cap: null, confidenceEffect: 'MISSING_LOCAL_CONTEXT', evidence: [], subject:null, severity:null };
  }

  if (level === 'SAFETY_OVERRIDE' || reasons.includes('SAFETY_RED_FLAG')) {
    return { type: 'SAFETY_OVERRIDE', adjustment: 0, cap: 39, confidenceEffect: 'DIRECT_CONSTRAINT', evidence: reasons, subject:null, severity:'SAFETY' };
  }
  if (reasons.some(code => ['PAIN_HIGH','DOMS_HIGH','GI_LIMITER_HIGH','CURRENT_CONSTRAINT_HIGH','ATHLETE_RESPONSE_MATERIAL_NEGATIVE'].includes(code))) {
    return { type: 'HIGH_CONSTRAINT', adjustment: -8, cap: 54, confidenceEffect: 'DIRECT_CONSTRAINT', evidence: reasons, subject:null, severity:'HIGH' };
  }
  if (reasons.some(code => ['PAIN_MODERATE','DOMS_MODERATE','ATHLETE_STATE_MATERIAL_NEGATIVE'].includes(code))) {
    return { type: 'MODERATE_CONSTRAINT', adjustment: -5, cap: 69, confidenceEffect: 'DIRECT_CONSTRAINT', evidence: reasons, subject:null, severity:'MODERATE' };
  }
  if (reasons.some(code => ['CONSTRAINT_REPORTED_RESOLVED','ATHLETE_RESPONSE_MATERIAL_POSITIVE','ATHLETE_STATE_MATERIAL_POSITIVE'].includes(code)) || /\b(fresh|fully recovered|no pain|no gi issues?|feeling (?:really )?good)\b/.test(summary)) {
    return { type: 'POSITIVE_CURRENT_STATE', adjustment: 3, cap: 95, confidenceEffect: 'DIRECT_CURRENT_STATE', evidence: reasons, subject:null, severity:null };
  }
  return { type: 'CURRENT_STATE_REPORTED', adjustment: 0, cap: null, confidenceEffect: 'DIRECT_CURRENT_STATE', evidence: reasons, subject:null, severity:null };
}
...
function localNarrative(modifier) {
  const subject=modifier?.subject?readableSubject(modifier.subject):null;
  if (modifier.type === 'SAFETY_OVERRIDE') return subject?`Current Athlete State contains a safety-level ${subject} constraint that overrides otherwise favourable physiology.`:'Current Athlete Voice contains a safety-level constraint that overrides otherwise favourable physiology.';
  if (modifier.type === 'HIGH_CONSTRAINT') return subject?`Current Athlete State contains a high ${subject} constraint that materially limits readiness.`:'Current Athlete Voice contains a high local/recovery constraint that materially limits readiness.';
  if (modifier.type === 'MODERATE_CONSTRAINT') return subject?`Current Athlete State contains a moderate ${subject} constraint that tempers readiness.`:'Current Athlete Voice contains a moderate local/recovery constraint that tempers readiness.';
  if (modifier.type === 'REPORTED_CONSTRAINT') return `Current Athlete State contains an active reported ${subject||'local'} constraint. It remains relevant to modality selection without automatically capping systemic readiness.`;
...
export function evaluateReadiness({ localDate, current = {}, history = [], athleteState = null, athleteEvent = null, materiality = null } = {}) {
  ...
  const modifier = readinessModifierFromAthleteContext({ athleteState, athleteEvent, materiality });
  ...
  return {
    ...
    rules: {
      ...
      canonicalActiveConstraintsGovernLocalState:true
    }
  };
}
...
export function mergeCanonicalReadinessIntoContext(context = {}, readiness = null) {
  ...
  const recovery = {
    ...
    constraintSeverity: readiness.modifier?.severity || context.recovery?.constraintSeverity || 'NONE',
