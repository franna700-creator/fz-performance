...
export async function persistRecommendationShadow({evaluation,context,trigger={}}={}){
  ...
  const payload={
    ...
    trigger:{
      ...
      temporalInvalidation:trigger.temporalInvalidation===true,
      contextFingerprintChanged:trigger.contextFingerprintChanged===true
    },
    contextSummary:{
      asOf:context?.asOf||localDate(),
      temporal:context?.temporal||null,
      athleteState:context?.athleteState?{
        engineVersion:context.athleteState.engineVersion||null,
        inputFingerprint:context.athleteState.inputFingerprint||null,
        localDate:context.athleteState.localDate||null,
        activeConstraints:(context.athleteState.activeConstraints||[]).map(item=>({subject:item.subject,severity:item.severity,lastConfirmedAt:item.lastConfirmedAt}))
      }:null,
      ...
      measurementEvidenceVersion:context?.measurement?.evidenceVersion||null,
      measurementEvidenceFingerprint:context?.measurement?.evidenceFingerprint||null,
