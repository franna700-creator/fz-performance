...
export function evaluateRecommendation(context={}){
  ...
  if(safety){
    ...
    const symptomLimiter=/not fully recovered|poor recovery|\bheavy\b|\bsore\b|\bill(?:ness)?\b|\bfatigue\b/.test(stateText);
    const structuredLocalLimiter=!['NONE','NO','CLEAR'].includes(constraintSeverity) && /\bconstraint\b|\blimiter\b/.test(localConstraintText);
