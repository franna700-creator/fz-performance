...
export function recommendationContextFingerprint(context={}){
  const decisionInput={
    ...
    measurement: context.measurement ? {
      ...
      evidenceVersion:context.measurement.evidenceVersion||null,
      evidenceFingerprint:context.measurement.evidenceFingerprint||null,
...
export function evaluateRecommendation(context={}){
  ...
  if(safety){
    ...
    explicitAbsorbState=/post[-\s]?session absorption|no further quality|recovery[-\s]?only|modify\s*\/\s*recover/.test(stateText);
  ...
  if(explicitAbsorbState && !safety){
    scores.ABSORB=Math.max(scores.ABSORB,1.5);
    scores.MAINTAIN=Math.min(scores.MAINTAIN,0.6);
    scores.ADAPT=Math.min(scores.ADAPT,0);
  }
  const lane=(safety||explicitAbsorbState)?'ABSORB':bestLane(scores);
  const fingerprint=recommendationContextFingerprint(context);
