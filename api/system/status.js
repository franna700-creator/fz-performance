...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [runtime, sources, evidence, wellness, athlete, materialityRows, shadowRows] = await Promise.all([
    ...
    return res.status(200).json({
      ...
      architecture:{operationalTruth:'Neon',sourceEvidence:['Garmin / Fitness AI','Tredict','Athlete Memory'],recommendationTruth:'Versioned FZ runtime/intelligence state',shadowRecommendationTruth:'Tranche 4.2 append-only FZ intelligence ledger',auditRepresentation:'Google Drive',driveRole:'human-owned audit / flight recorder; not runtime engine',runtimeStoreMode:runtimeStoreMode()},
      ...
      intelligence:{
        ...
        recommendationShadow:{engineVersion:RECOMMENDATION_ENGINE_VERSION,mode:RECOMMENDATION_ENGINE_MODE,activeRecommendationWrite:false,todayActivation:false,count:shadowRows.length,evaluations:shadowRows.map(row=>({id:row.id,localDate:row.local_date,evaluatedAt:row.source_updated_at||row.ingested_at,recommendationId:row.payload?.recommendationId||null,status:row.payload?.status||null,lane:row.payload?.lane||null,confidence:row.payload?.confidence||null,trigger:row.payload?.trigger||null,contextSummary:row.payload?.contextSummary||null,explanation:row.payload?.explanation||null}))}
      }
    });
  } catch (error) { console.error('FZ system status failed', error instanceof Error ? error.message : String(error)); return res.status(503).json({ok:false,error:'system_status_unavailable',detail:error instanceof Error?error.message:String(error)}); }
}
