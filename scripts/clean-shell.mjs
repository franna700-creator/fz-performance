...
html=html
  .replace(/<meta content="[^"]+" name="fz-build"\/>/, '<meta content="2026-09-11-v0.8.0-rc1-ui-recovery" name="fz-build"/><meta content="4.3-context-recovery" name="fz-shell"/>')
  ...
  .replace(/<div class="side-note">[\s\S]*?<\/div><\/aside>/, '<div class="side-note">v0.8 · Neon operational truth · 4.3 active recommendation · contextual UI recovery · fail-stale PWA.</div></aside>')
...
for(const asset of ['/assets/app-clean.js','/assets/clean.css','/assets/live-physiology.js','/assets/live-physiology.css','/assets/training-auto-sync.js','/assets/training-auto-sync.css','/assets/system-intelligence.js','/assets/intelligence-refresh.js','/assets/fz-design-system.css','/assets/ui-context-recovery.js','/assets/ui-context-recovery.css']) if(!html.includes(asset))throw new Error(`Clean runtime asset not wired: ${asset}`);
...
fs.writeFileSync(path.join(dist,'release-ui-contract.json'),JSON.stringify({shell:'4.3-context-recovery',canonicalRuntime:true,legacyAppJs:false,requiredAssets:['app-clean.js','intelligence-refresh.js','ui-context-recovery.js','ui-context-recovery.css']},null,2));
console.log('PASS clean shell v0.8.0-rc1 UI recovery: canonical intelligence + athlete context + bounded Trends; legacy static shell removed');
