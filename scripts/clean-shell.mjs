...
for(const input of [htmlPath,cssPath,cleanJs,cleanCss,layoutStabilityJs,liveJs,liveCss,trainingSyncJs,trainingSyncCss,systemIntelligenceJs,intelligenceRefreshJs,trainingMemoryRichJs,adaptiveChoiceJs,adaptiveChoiceCss,fzDesignCss,uiContextJs,uiContextCss])if(!fs.existsSync(input))throw new Error(`Clean shell input missing: ${input}`);
...
html=html
  .replace(/<meta content="[^"]+" name="fz-build"\/>/, '<meta content="2026-09-14-v0.8.0-rc4-readiness-roadmap" name="fz-build"/><meta content="4.5-readiness-outcome-prep" name="fz-shell"/>')
  ...
  .replace(/<div class="side-note">[\s\S]*?<\/div><\/aside>/, '<div class="side-note">v0.8 · Neon operational truth · canonical readiness · 4.4 concrete session options · 4.5 outcome observation.</div></aside>')
...
fs.writeFileSync(path.join(dist,'release-ui-contract.json'),JSON.stringify({shell:'4.5-readiness-outcome-prep',canonicalRuntime:true,executableGoldenThreads:true,canonicalTrainingMetrics:true,canonicalReadiness:true,sessionOptionComposer:true,athleteChoiceCanonical:true,plannedIntentReconciliation:true,choiceOutcomeObservation:true,crossSurfaceRationalised:true,layoutStabilityGuard:true,legacyAppJs:false,requiredAssets:['layout-stability.js','app-clean.js','intelligence-refresh.js','training-memory-rich.js','adaptive-choice.js','adaptive-choice.css','ui-context-recovery.js','ui-context-recovery.css']},null,2));
console.log('PASS clean shell v0.8.0-rc4: canonical readiness + 4.4 choice + 4.5 outcome observation + viewport stability');
