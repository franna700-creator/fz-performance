...
const layoutStabilityJs=path.join(root,'src','layout-stability.js');
...
const todayExperienceJs=path.join(root,'src','today-experience.js');
const todayExperienceCss=path.join(root,'src','today-experience.css');
for(const input of [htmlPath,cssPath,cleanJs,cleanCss,layoutStabilityJs,liveJs,liveCss,trainingSyncJs,trainingSyncCss,systemIntelligenceJs,intelligenceRefreshJs,trainingMemoryRichJs,adaptiveChoiceJs,adaptiveChoiceCss,fzDesignCss,uiContextJs,uiContextCss,todayExperienceJs,todayExperienceCss])if(!fs.existsSync(input))throw new Error(`Clean shell input missing: ${input}`);
...
for(const asset of ['/assets/layout-stability.js','/assets/app-clean.js','/assets/clean.css','/assets/live-physiology.js','/assets/live-physiology.css','/assets/training-auto-sync.js','/assets/training-auto-sync.css','/assets/system-intelligence.js','/assets/intelligence-refresh.js','/assets/training-memory-rich.js','/assets/adaptive-choice.js','/assets/adaptive-choice.css','/assets/fz-design-system.css','/assets/ui-context-recovery.js','/assets/ui-context-recovery.css','/assets/today-experience.js','/assets/today-experience.css'])if(!html.includes(asset))throw new Error(`Clean runtime asset not wired: ${asset}`);
...
if(html.indexOf('/assets/today-experience.js')<html.indexOf('/assets/ui-context-recovery.js'))throw new Error('TODAY presentation layer must load after canonical/adaptive UI modules');
fs.writeFileSync(htmlPath,html);
for(const [source,name] of [[layoutStabilityJs,'layout-stability.js'],[cleanJs,'app-clean.js'],[cleanCss,'clean.css'],[liveJs,'live-physiology.js'],[liveCss,'live-physiology.css'],[trainingSyncJs,'training-auto-sync.js'],[trainingSyncCss,'training-auto-sync.css'],[systemIntelligenceJs,'system-intelligence.js'],[intelligenceRefreshJs,'intelligence-refresh.js'],[trainingMemoryRichJs,'training-memory-rich.js'],[adaptiveChoiceJs,'adaptive-choice.js'],[adaptiveChoiceCss,'adaptive-choice.css'],[fzDesignCss,'fz-design-system.css'],[uiContextJs,'ui-context-recovery.js'],[uiContextCss,'ui-context-recovery.css'],[todayExperienceJs,'today-experience.js'],[todayExperienceCss,'today-experience.css']])fs.copyFileSync(source,path.join(dist,'assets',name));
