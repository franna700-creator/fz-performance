...
if(!fs.existsSync(htmlPath)||!fs.existsSync(cssPath)||!fs.existsSync(cleanJs)||!fs.existsSync(cleanCss)||!fs.existsSync(liveJs)||!fs.existsSync(liveCss)||!fs.existsSync(trainingSyncJs)||!fs.existsSync(trainingSyncCss)||!fs.existsSync(systemIntelligenceJs))throw new Error('Clean shell inputs missing');
...
html=html
  .replace(/<meta content="[^"]+" name="fz-build"\/>/, '<meta content="2026-09-10-v0.7.0-rc1" name="fz-build"/>')
...
for(const asset of ['/assets/app-clean.js','/assets/clean.css','/assets/live-physiology.js','/assets/live-physiology.css','/assets/training-auto-sync.js','/assets/training-auto-sync.css','/assets/system-intelligence.js']){
...
console.log('PASS clean shell v0.7.0-rc1: dynamic physiology + training + 4.1 observability + runtime-owned TODAY/TRENDS/TRAIN/SYSTEM');
