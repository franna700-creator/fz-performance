...
assert.match(systemApi, /releaseEnvironment:\{databaseConfigured:databaseConfigured\(\),writeTokenConfigured:Boolean\(process\.env\.FZ_STATE_WRITE_TOKEN\),athleteBootstrapConfigured:Boolean\(process\.env\.FZ_ATHLETE_BOOTSTRAP_TOKEN\),intervalsIcuConfigured:intervalsIcuConfigured\(\),garminCiqConfigured:garminCiqConfigured\(\)/, 'SYSTEM must expose secret-safe release environment probes including Athlete Mode bootstrap and Garmin CIQ readiness');
...
assert.ok(environmentNames.has('FZ_CIQ_INGEST_TOKEN'), 'Garmin Connect IQ ingest token must be declared');
