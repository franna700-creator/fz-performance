...
// The visual explorer retains a stable historical seed, then layers the validated current
// runtime state on top. This keeps the shell immutable while making the latest overnight point,
// FZ readiness and personal baseline calculations current after every state publish.
const liveHelpers=String.raw`
function fzRuntimeWellnessHistory(){
 const rows=FZ_WELLNESS_HISTORY.map(r=>[...r]);
 ...
 const parts=stateId.slice(0,10).split('-').map(Number),months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
 if(parts.length!==3||!months[parts[1]-1])return rows;
 const label=String(parts[2]).padStart(2,'0')+' '+months[parts[1]-1];
 ...
 if(last?.[0]===label)rows[rows.length-1]=row;else{if(last?.[10]==='LIVE / PARTIAL')last[10]='HISTORICAL';rows.push(row)}
 return rows;
}
...
function fzRuntimeWellnessDef(metric){
 ...
 if(['hrv','rhr','sleep','sleepScore','bb'].includes(metric)){
   const vals=rows.slice(0,-1).map(r=>r[def.idx]).filter(Number.isFinite);
...
function fzRuntimeLongitudinalData(){
 ...
 const ids={'HRV':'hrv','Resting HR':'rhr','Sleep duration':'sleep','Sleep score':'sleepScore','Body Battery high':'bb','Completed-day stress':'stress','Completed-day steps':'steps','FZ Readiness':'readiness'};
...
mustReplace(
 ...
 "const labelIdx=[0,5,10,15,history.length-1].filter((v,i,a)=>v<history.length&&a.indexOf(v)===i);const labels=labelIdx.map(i=>'<text x=\"'+x(i)+'\" y=\"'+(H-12)+'\" class=\"well-axis\" text-anchor=\"middle\">'+history[i][0]+'</text>').join('');",
...
if(!app.includes('existingLong.remove()')||!app.includes("data-long-lens=\"state\"")||!app.includes('fzRuntimeWellnessHistory')||!app.includes('fzInjectCurrentTrendSummaries')||!app.includes('fzRenderWellnessChart')){
...
console.log('TRENDS primary surface: interactive longitudinal layer + live runtime wellness sync');
